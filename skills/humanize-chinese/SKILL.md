...
默认情况下，humanize-chinese 保持中立的去 AI 味润色，不套任何作者的腔调，按 `## Core Rules` 走。
...
- 不要把作者档案的反模式当成 humanize-chinese 的默认规则；只在该声音生效的轮次中应用。
...
- 引号样式按以下顺序确定：用户在本轮请求里明确要求 → 项目 `CLAUDE.md`/`AGENTS.md` 的声明（例如 `humanize-chinese quotes: 「」`） → 默认 `""`。
